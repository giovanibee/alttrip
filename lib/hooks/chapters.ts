...
const filterChaptersByDistance = (chapters: Chapter[], location: LatLngTuple) => {
	...
	useQuery({
		...
		queryFn: async (): Promise<SortedChapters | null> => {
			try {
				...
				}).json() as { res: SortedChapters }).res

				if (!response) throw new Error('No response')
				if (shouldFilterByDistance && location) {
					response.incompleteChapters = filterChaptersByDistance(response.incompleteChapters, location)
					response.completedChapters = filterChaptersByDistance(response.completedChapters, location)
				}
				return response
			} catch (error) {
				console.error(error)
...
export const useGetSecretText = () => useMutation({
		...
		useMutation({
			mutationFn: async ({ chapterId, passcode }: VerifyPasscodeProps) => {
				try {
					...
					const secretText = ((await response.json()) as { res: string | null })?.res
					if (!secretText) {
